...
plt.savefig('/output/figure8.png')
...
plt.savefig('/output/figure9.png')


tsne_data = np.load('/output/tsne_2d_30p.npz')
...
fig.savefig('/output/figure10.png')


tsne_data = np.load('/output/tsne_3d_30p.npz')
tsne_train = tsne_data['train']
tsne_valid = tsne_data['valid']
tsne_test = tsne_data['test']
tsne_all = np.concatenate([tsne_train, tsne_valid, tsne_test], axis=0)
...
fig.savefig('/output/figure11.png')
...
plotly.offline.plot(go.Figure(data=[trace], layout=go.Layout(title='tsne-3d-scatter')), filename='tsne-3d-scatter.html')
